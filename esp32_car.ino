...
#define SIGNAL_TIMEOUT 1000  // Signal timeout in milliseconds. Reset to STOP if no signal
unsigned long lastRecvTime = 0;

// Function prototype so processCarMovement knows it exists
...
void setUpPinModes()
{
  for (size_t i = 0; i < motorPins.size(); i++)
  {
    ...
    //Set up PWM for motor speed (Compatible with ESP32 Core v2.x.x)
...
void loop() 
{
  ...
  if (Serial1.available())
  {
    ...
    // Reset the timeout timer whenever a valid command is received
    lastRecvTime = millis();   
  }

  // Safety Timeout: Stop the car if no signal has been received for SIGNAL_TIMEOUT milliseconds
