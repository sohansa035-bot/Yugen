...
{                      
  switch (type) 
  {
    ...
    case WS_EVT_DISCONNECT:
      ...
      // FIXED: Use ledcWrite(PIN, value) instead of ledcWrite(CHANNEL, value)
...
void setUpPinModes()
{
  // FIXED: New LEDC API for ESP32 Core v3.0+
...
void setup(void) 
{
  ...
  // ============================================
  // START OF NEW SECTION: Computer Vision Input
  // ============================================
  server.on("/action", HTTP_GET, [](AsyncWebServerRequest *request){
    if(request->hasParam("go")){
        String command = request->getParam("go")->value();

        // Debug to Serial Monitor
        Serial.println("Command Received: " + command);

        // Convert command to logic used by sendCarCommands
        // 1=Forward, 2=Backward, 3=Left, 4=Right, 0=Stop
        if(command == "left") {
            sendCarCommands("MoveCar,3"); 
        }
        else if(command == "right") {
            sendCarCommands("MoveCar,4");
        }
        else if(command == "stop") {
            sendCarCommands("MoveCar,0");
        }
        else if(command == "forward") {
            sendCarCommands("MoveCar,1");
        }
        else if(command == "backward") {
            sendCarCommands("MoveCar,2");
        }
    }
